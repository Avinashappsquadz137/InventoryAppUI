...
struct ReturnItemChallanId : Codable {
    let status : Bool?
    let message : String?
    let data : String?
    let error : ChallanIdError?
    ...
    init(from decoder: Decoder) throws {
        ...
        message = try values.decodeIfPresent(String.self, forKey: .message)
        data = try values.decodeIfPresent(String.self, forKey: .data)
        error = try values.decodeIfPresent(ChallanIdError.self, forKey: .error)
    }

}

struct ChallanIdError : Codable {
    let error_code : Int?
    let error_message : String?
